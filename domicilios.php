...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $_POST['action'] === 'create') {
    ...
    $producto_q = pg_query_params($conn, "SELECT nombre, precio FROM productos WHERE id_producto = $1", [$id_producto]);
    ...
    if ($producto_data && $cantidad > 0) {
        ...
        if ($ok) {
            ...
            pg_query_params($conn, $mov, [$id_producto, $cantidad, $precio_unitario, $desc]);
        }
...
// === CARGAR PRODUCTOS ===
$productos = pg_query($conn, "SELECT id_producto, nombre, stock FROM productos ORDER BY nombre ASC");
